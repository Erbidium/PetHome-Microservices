...
export default function SignUpView() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!formData.id.trim() || !formData.email.trim() || !formData.password.trim()) {
        alert('Please fill in all fields');
        return;
    }

    AuthService.signUp(formData).then(data => {
      setResponseText(`Sign up successful: ${JSON.stringify(data.data)}`);
  ...
  return (
    ...
    <form onSubmit={handleSubmit}>
        ...
        <MyInput type="text" id="id" name="id" value={formData.id} onChange={handleChange} />
